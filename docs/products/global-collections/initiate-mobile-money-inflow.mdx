...
| `Failed to initiate mobile money inflow` | The request was rejected for another reason. | Check the request, then retry with a **new** `idempotencyKey`. |
...
Treat these descriptions as the contract; do not parse anything else out of the response.
