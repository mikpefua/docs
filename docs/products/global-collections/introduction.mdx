...
`gatewayMessage` is the provider's own word. `gatewayCode` says what it means:
...
The providers do not give a reason for a decline. In practice the usual cause is an unfunded
customer wallet, and the customer is prompted for the amount **plus the provider's fee**, not the
amount alone -- so retrying at the same amount often declines again.
